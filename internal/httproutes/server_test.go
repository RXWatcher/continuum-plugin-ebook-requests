...
// Numeric/bool query params must arrive as their scalar form, not the
// protobuf debug text ("number_value:50"), or pagination/sort silently break.
...
// A malformed method from the (untrusted) RPC payload must not panic — the
// old httptest.NewRequest path crashed the serving goroutine.
func TestHandle_BadMethodNoPanic(t *testing.T) {
	...
	for _, m := range []string{"G ET", "GET\r\nX: y", "\x00", "POST /x"} {
		...
		if err != nil {
			t.Fatalf("Handle returned error (want graceful response) for %q: %v", m, err)
		}
...
func TestHandle_BodyTooLarge(t *testing.T) {
	...
	resp, err := s.Handle(context.Background(), &pluginv1.HandleHTTPRequest{
		Method: "POST", Path: "/api/v1/external_search",
		Body: []byte(strings.Repeat("x", maxBodyBytes+1)),
	})
	if err != nil {
		t.Fatalf("Handle: %v", err)
	}
